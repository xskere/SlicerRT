...
const int qSlicerIpoptOptimizer::DEFAULT_MAX_ITER = 3000;
  ...
  const QString& name, vtkMRMLRTObjectiveNode* node)
{
  ...
  if      (name == "Min/Max Dose Constraint") c = std::make_unique<qSlicerMinMaxDoseConstraint>();
  else if (name == "DVH Constraint")      c = std::make_unique<qSlicerMinMaxDVHConstraint>();
  else if (name == "EUD Constraint")      c = std::make_unique<qSlicerMinMaxEUDConstraint>();
  else if (name == "Mean Dose Constraint") c = std::make_unique<qSlicerMinMaxMeanDoseConstraint>();
  else return nullptr;
